                  ...
                  () => Column(
                    ...
                    children: [
                          ...
                          children: [
                            Expanded(
                              child: Container(
                                ...
                                child: Obx(() {
                                  final selected =
                                      jobC.selectedImageBeforeEdit.value;
                                  final imageBeforeUrl =
                                      jobC.imageBeforeC.value;
                                  if (selected != null) {
                                    return ClipRRect(
                                      borderRadius: BorderRadius.circular(12),
                                      child: Image.file(
                      ...
                      if (jobC.selectedImageBeforeEdit.value != null ||
                          jobC.imageBeforeC.value.isNotEmpty) ...[
                        Padding(
                      ...
                      if (jobC.selectedImageAfterEdit.value != null ||
                          jobC.imageAfterC.value.isNotEmpty) ...[
                        Padding(
                          padding: const EdgeInsets.only(top: 6, left: 4),
                          child: Text(
                            '* Tekan untuk melihat pratinjau',
                                ...
                                child: ElevatedButton.icon(
                                  ...
                                  onPressed: () async {
                                    AppDialog.confirm(
                                      ...
                                      icon: Icons.delete,
                                      onConfirm: () async {
                                        final result = await jobC.deleteById(
                                          jobId,
                                        );
                                        if (result == 'ok') {
                                          Get.back(result: true);
                                          AppSnackbarRaw.success(
                                            'Berhasil hapus data pekerjaan!',
                                          );
                                        } else {
                                          AppSnackbarRaw.error(result);
                                        }
                          ...
                          suffixIcon: IconButton(
                            onPressed: () async {
                              if (textCommentC.text.isNotEmpty) {
                                final result = await commentC.create(
                                  jobId,
                                  textCommentC.text,
                                );
                                if (result == 'ok') {
